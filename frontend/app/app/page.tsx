...
interface Turn {
  ...
  /** True while tokens are still arriving — drives the typing cursor. */
  streaming: boolean;
  error: string | null;
}
...
export default function Home() {
  ...
  const submit = async (text: string) => {
    ...
    try {
      ...
      await askStream(trimmed, selectedIds.size ? [...selectedIds] : null, (event) => {
        switch (event.type) {
          case "meta":
            // Mode and the retrieved sources are known before the text arrives.
            patch((t) => ({ ...t, mode: event.mode, citations: event.citations, answer: "" }));
            break;
          case "delta":
            patch((t) => ({ ...t, answer: (t.answer ?? "") + event.text }));
            break;
          case "done":
            // Swap in the server's validated text and keep only cited sources —
            // invented markers are stripped there, so nothing dead is rendered.
            patch((t) => ({
              ...t,
              answer: event.answer,
              abstained: event.abstained,
              streaming: false,
              citations: event.cited_markers.length
                ? t.citations.filter((c) => event.cited_markers.includes(c.marker))
                : [],
            }));
            break;
          case "error":
            patch((t) => ({ ...t, error: event.message, streaming: false }));
            break;
        }
      });
    } catch (err) {
      ...
      // The stream may end without a `done` event (network drop) — make sure the
      // cursor stops blinking either way.
  ...
  return (
    <main className="flex h-screen overflow-hidden">
      ...
      {/* Chat */}
      <section className="flex min-w-0 flex-1 flex-col border-r border-slate-200 bg-gradient-to-b from-white via-white to-clinical-50/30 dark:border-slate-700/60 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800/40">
        <header className="flex items-center justify-between border-b border-slate-200/70 bg-white/70 px-6 py-3 backdrop-blur-md dark:border-slate-700/60 dark:bg-slate-900/60">
          <div>
            <h2 className="text-sm font-semibold text-slate-800 dark:text-slate-100">Conversation</h2>
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full bg-clinical-50 px-2.5 py-1 text-xs font-medium text-clinical-700 dark:bg-clinical-500/15 dark:text-clinical-300">
              ...
              <div className="mt-3 grid gap-2.5 sm:grid-cols-2">
                {SAMPLE_QUESTIONS.map((q, i) => (
                  <button
                    ...
                    className="group flex animate-fade-up items-start gap-2.5 rounded-xl border border-slate-200 bg-white p-3 text-left text-sm text-slate-700 shadow-card transition hover:-translate-y-0.5 hover:border-clinical-400 hover:shadow-float dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:border-clinical-400"
                  >
              ...
              {turns.map((turn) => (
                <div key={turn.id} className="animate-fade-up">
                  ...
                  {turn.error ? (
                    ...
                    <div className="rounded-2xl rounded-tl-md border border-slate-200/80 bg-white p-4 shadow-card dark:border-slate-700 dark:bg-slate-800">
                      {turn.mode === "general" && (
                          ...
                          <div className="space-y-2">
                            {turn.citations.map((citation) => (
                              <button
                                ...
                                className="group block w-full rounded-xl border border-slate-200 bg-slate-50/60 px-3 py-2.5 text-left transition hover:-translate-y-0.5 hover:border-clinical-400 hover:bg-white hover:shadow-card dark:border-slate-700 dark:bg-slate-900/50 dark:hover:bg-slate-900"
                              >
                                <div className="flex items-baseline gap-2">
                                  ...
                                  <span className="ml-auto shrink-0 text-xs font-medium text-clinical-600 dark:text-clinical-300">
        ...
        <form
          ...
          className="border-t border-slate-200/70 bg-white/70 px-6 py-4 backdrop-blur-md dark:border-slate-700/60 dark:bg-slate-900/60"
        >
          <div className="mx-auto flex max-w-2xl items-center gap-1.5 rounded-full border border-slate-300 bg-white py-1.5 pl-4 pr-1.5 shadow-card transition focus-within:border-clinical-500 focus-within:shadow-float focus-within:ring-2 focus-within:ring-clinical-500/20 dark:border-slate-600 dark:bg-slate-800">
            <input
              value={question}
              onChange={(e) => setQuestion(e.target.value)}
              placeholder="Ask about a diagnosis, medication, lab value…"
              disabled={pending}
              className="flex-1 bg-transparent py-1.5 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none disabled:opacity-60 dark:text-slate-100 dark:placeholder:text-slate-500"
            />
            <button
              type="submit"
          ...
          <p className="mx-auto mt-2 max-w-2xl text-center text-[11px] text-slate-400 dark:text-slate-500">
